...
- **`stage_id`** — выводится из поля `CLOSED`: `'Y'` → `'Завершено'`, иначе → `'В роботі'`. В Bitrix нет отдельного поля "статус проекта" — `CLOSED` единственный признак завершённости.
- **`type`** — выводится из поля `PROJECT`: `'Y'` → `'Проект'`, иначе → `'Робоча група'`. В Odoo это поле не является стандартным атрибутом `project.project` — при импорте нужно уточнить, куда его маппить (кастомное поле или игнорировать).
