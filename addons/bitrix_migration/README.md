...
- Вкладка `Help` в Odoo содержит короткую operational-инструкцию по режимам и опциям.
- Опция `Fallback System Author` скрыта из формы (UI), так как в текущей версии не влияет на поведение загрузчика комментариев.
